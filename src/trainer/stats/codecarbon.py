...
def construct_trainer_stats(conf: config.Config, **kwargs) -> base.TrainerStats:
    if "device" in kwargs:
        ...
        logger.warning(
            "No device provided to codecarbon trainer stats. Using default PyTorch device"
        )
    ...
    return CodeCarbonStats(
        ...
        conf.trainer_stats_configs.codecarbon.measure_power_secs,
    )
...
class CodeCarbonStats(base.TrainerStats):
    ...
    def __init__(
        ...
        # GPU ranks - wrap in torch.device (None when running on CPU)
        ...
        self.total_training_tracker = OfflineEmissionsTracker(
            ...
            output_handlers=[
                SimpleFileOutput(
                    output_file_name=f"{run_number}cc_full_rank_{gpu_id}.csv",
        ...
        self.training_step_tracker = OfflineEmissionsTracker(
            ...
            output_handlers=[
                SimpleFileOutput(
                    output_file_name=f"{run_number}cc_step_rank_{gpu_id}.csv",
                    output_dir=output_dir,
                )
            ],
            allow_multiple_runs=True,
        ...
        self.training_substep_tracker = OfflineEmissionsTracker(
            ...
            output_handlers=[
                SimpleFileOutput(
                    output_file_name=f"{run_number}cc_substep_rank_{gpu_id}.csv",
                    output_dir=output_dir,
                )
            ],
            allow_multiple_runs=True,
    ...
    def start_forward(self) -> None:
        if self.device.type == "cuda":
            torch.cuda.synchronize(self.device)
        self.training_substep_tracker.start_task(
            task_name=f"Forward pass #{self.iteration}"
        )

    def stop_forward(self) -> None:
        if self.device.type == "cuda":
            torch.cuda.synchronize(self.device)
        self.training_substep_tracker.stop_task(
            task_name=f"Forward pass #{self.iteration}"
        )
    ...
    def start_backward(self) -> None:
        if self.device.type == "cuda":
            torch.cuda.synchronize(self.device)
        self.training_substep_tracker.start_task(
            task_name=f"Backward pass #{self.iteration}"
        )
    ...
    def start_optimizer_step(self) -> None:
        if self.device.type == "cuda":
            torch.cuda.synchronize(self.device)
        self.training_substep_tracker.start_task(
            task_name=f"Optimisation step #{self.iteration}"
        )
    ...
    def log_stats(self) -> None:
        ...
        """
        # losses as dataframe with named columns
        df = pd.DataFrame(
            [[x["task_name"], x["loss"].item()] for x in self.losses],
            columns=["task_name", "loss"],
        )
        ...
        save_file_path = os.path.join(
            losses_dir, f"{run_number}cc_loss_rank_{gpu_id}.csv"
        )
        df.to_csv(save_file_path, index=False)

        logger.info(
            f"CODECARBON LOSS LOGGING: Rank {gpu_id} - Run {self.run_num} - Losses saved to {save_file_path}"
        )
